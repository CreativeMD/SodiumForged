...
public class CloudRenderer {
    ...
    private void applyFogModifiers(ClientLevel world, FogRenderer.FogData fogData, LocalPlayer player, int cloudDistance, float tickDelta) {
        ...
        } else if (world.effects().isFoggyAt(Mth.floor(camera.getPosition().x), Mth.floor(camera.getPosition().z)) || Minecraft.getInstance().gui.getBossOverlay()
                .shouldCreateWorldFog()) {
            fogData.start = (cloudDistance) * 0.05f;
            fogData.end = Math.min((cloudDistance), 192.0f) * 0.5f;
        }
        
        FogRenderer.MobEffectFogFunction fogModifier = BackgroundRendererInvoker.invokeGetPriorityFogFunction(player, tickDelta);
        if (fogModifier != null) {
