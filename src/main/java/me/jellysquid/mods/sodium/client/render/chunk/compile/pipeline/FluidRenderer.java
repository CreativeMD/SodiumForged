...
public class FluidRenderer {
    ...
    private boolean isFluidOccluded(BlockAndTintGetter world, int x, int y, int z, Direction dir, Fluid fluid) {
        ...
        if (blockState.canOcclude()) {
            return world.getFluidState(adjPos).getFluid().isSame(fluid) || blockState.isFaceSturdy(world, pos, dir, SupportType.FULL);
        }
        return world.getFluidState(adjPos).getFluid().isSame(fluid);
    }
    ...
    private boolean isSideExposed(BlockAndTintGetter world, int x, int y, int z, Direction dir, float height) {
        ...
        if (blockState.canOcclude()) {
            ...
            VoxelShape threshold = Shapes.create(0.0D, 0.0D, 0.0D, 1.0D, height, 1.0D);
    ...
    public void render(WorldSlice world, FluidState fluidState, BlockPos blockPos, BlockPos offset, ChunkBuildBuffers buffers) {
        ...
        if (fluidHeight >= 1.0f) {
            ...
            var scratchPos = new MutableBlockPos();
            ...
            northWestHeight = this.fluidCornerHeight(world, fluid, fluidHeight, heightNorth, heightWest,
                    scratchPos.set(blockPos).move(Direction.NORTH).move(Direction.WEST));
            southWestHeight = this.fluidCornerHeight(world, fluid, fluidHeight, heightSouth, heightWest,
                    scratchPos.set(blockPos).move(Direction.SOUTH).move(Direction.WEST));
            southEastHeight = this.fluidCornerHeight(world, fluid, fluidHeight, heightSouth, heightEast,
                    scratchPos.set(blockPos).move(Direction.SOUTH).move(Direction.EAST));
            northEastHeight = this.fluidCornerHeight(world, fluid, fluidHeight, heightNorth, heightEast,
        ...
        if (!sfUp && this.isSideExposed(world, posX, posY, posZ, Direction.UP,
            ...
            float s1 = sprites[0].contents().width() / (sprites[0].getU1() - sprites[0].getU0());
            float s2 = sprites[0].contents().height() / (sprites[0].getV1() - sprites[0].getV0());
